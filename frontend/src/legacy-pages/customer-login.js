const page = {
  title: `Customer Login - Saranya Jewellery`,
  html: `<div class="top-bar">
        <div><i class="fas fa-phone"></i> <a href="tel:+1234567890">Contact Us</a></div>
        <div></div>
    </div>

    <header class="header">
        <div class="nav">
            <a href="/">Home</a>
            <a href="/customer-shop">Shop</a>
            <a href="/customer-orders" data-requires-auth="true">My Orders</a>
            <a href="/customer-loyalty" data-requires-auth="true">Loyalty</a>
            <a href="/customer-support" data-requires-auth="true">Support</a>
        </div>

        <div class="logo">SARANYA JEWELLERY</div>

        <div class="header-icons">
            <i class="fas fa-search header-icon"></i>
            <a href="/customer-login" id="userIcon"><i class="fas fa-user header-icon"></i></a>
            <a href="/customer-cart" style="position: relative;">
                <i class="fas fa-shopping-cart header-icon"></i>
                <span id="cartBadge" style="display: none; position: absolute; top: -8px; right: -8px; background: var(--brand-gold-strong); color: white; border-radius: 50%; padding: 2px 6px; font-size: 0.7rem;">0</span>
            </a>
        </div>
    </header>

    <main>
        <div class="login-container" style="margin-top: 2rem; margin-bottom: 3rem;">
            <div class="logo-area">
                <h1>Saranya</h1>
                <p>Customer Login</p>
            </div>

            <div id="alert" class="alert"></div>

            <form id="loginForm">
                <div class="form-group">
                    <label for="email">Email Address</label>
                    <input type="email" id="email" name="email" placeholder="Enter your email" required>
                </div>

                <div class="form-group">
                    <label for="password">Password</label>
                    <input type="password" id="password" name="password" placeholder="Enter your password" required>
                </div>

                <button type="submit" class="btn" id="loginBtn">Login</button>
            </form>

            <div class="footer-links">
                <a href="/customer-register">Create Account</a>
                <span class="divider">|</span>
                <a href="/">Back to Home</a>
            </div>
        </div>
    </main>

    <footer class="footer">
        <div class="footer-content">
            <div class="footer-section">
                <h3>ABOUT SARANYA</h3>
                <ul>
                    <li><a href="/#story">Our Story</a></li>
                    <li><a href="/#education">Education</a></li>
                    <li><a href="/#faq">FAQ</a></li>
                </ul>
            </div>

            <div class="footer-section">
                <h3>WHY SARANYA?</h3>
                <ul>
                    <li><a href="/#quality">Quality Guarantee</a></li>
                    <li><a href="/#warranty">Lifetime Warranty</a></li>
                    <li><a href="/#certification">Certified Jewellery</a></li>
                    <li><a href="/#shipping">Free Shipping</a></li>
                </ul>
            </div>

            <div class="footer-section">
                <h3>SERVICES</h3>
                <ul>
                    <li><a href="/#custom">Custom Design</a></li>
                    <li><a href="/#resize">Free Ring Resize</a></li>
                    <li><a href="/#gift">Gift Services</a></li>
                </ul>
            </div>

            <div class="footer-section">
                <h3>POLICIES</h3>
                <ul>
                    <li><a href="/#policies">Our Policies</a></li>
                    <li><a href="/#privacy">Privacy Policy</a></li>
                    <li><a href="/#terms">Terms & Conditions</a></li>
                    <li><a href="/#returns">Return Policy</a></li>
                    <li><a href="/#payment">Payment Methods</a></li>
                </ul>
            </div>

            <div class="footer-section">
                <h3>CONNECT WITH US</h3>
                <div class="social-links">
                    <a href="#" class="social-icon"><i class="fab fa-facebook-f"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-instagram"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-twitter"></i></a>
                    <a href="#" class="social-icon"><i class="fab fa-pinterest"></i></a>
                </div>
            </div>
        </div>

        <div class="footer-bottom">
            <p>&copy; 2026 Saranya Jewellery. All Rights Reserved.</p>
        </div>
    </footer>`,
  scripts: [
    {
      type: `module`,
      content: `
        import authManager from '/src/auth.js';
        
        const loginForm = document.getElementById('loginForm');
        const loginBtn = document.getElementById('loginBtn');
        const alertDiv = document.getElementById('alert');
        const registerLink = document.querySelector('a[href="/customer-register"]');
        const userIcon = document.getElementById('userIcon');
        const cartBadge = document.getElementById('cartBadge');

        function setActiveNavLink() {
            const currentPath = window.location.pathname;
            document.querySelectorAll('.nav a').forEach((link) => {
                const href = link.getAttribute('href');
                if (!href || href.startsWith('#')) return;
                if (href === currentPath) {
                    link.classList.add('active');
                }
            });
        }

        function attachProtectedNavHandlers() {
            document.querySelectorAll('.nav a[data-requires-auth="true"]').forEach((link) => {
                link.addEventListener('click', (event) => {
                    event.preventDefault();
                    const target = link.getAttribute('href') || '/customer-dashboard';
                    authManager.redirectToLogin('customer', { returnTo: target });
                });
            });
        }

        function getSafeRedirectPath() {
            const redirectParam = new URLSearchParams(window.location.search).get('redirect');
            if (!redirectParam) return '/customer-dashboard';

            // Only allow same-origin relative paths to prevent open redirects
            if (!redirectParam.startsWith('/') || redirectParam.startsWith('//')) {
                return '/customer-dashboard';
            }

            if (redirectParam.startsWith('/customer-login')) {
                return '/customer-dashboard';
            }

            return redirectParam;
        }

        const redirectAfterLogin = getSafeRedirectPath();

        if (registerLink) {
            registerLink.href = \`/customer-register?redirect=\${encodeURIComponent(redirectAfterLogin)}\`;
        }

        function updateCartBadge() {
            const cart = JSON.parse(localStorage.getItem('saranyaCart') || '[]');
            const count = cart.reduce((sum, item) => sum + item.quantity, 0);

            if (count > 0) {
                cartBadge.textContent = count;
                cartBadge.style.display = 'block';
            } else {
                cartBadge.style.display = 'none';
            }
        }

        async function updateLoginHeaderState() {
            try {
                const res = await authManager.apiRequest('/api/customer/me');
                if (!res.ok) return;

                const data = await res.json();
                if (data.customer) {
                    userIcon.href = '/customer-dashboard';
                    userIcon.title = data.customer.fullName || data.customer.email;
                }
            } catch (error) {
                // Keep defaults for guest users
            }
        }

        function showAlert(message, type = 'error') {
            alertDiv.textContent = message;
            alertDiv.className = \`alert alert-\${type} show\`;
            setTimeout(() => {
                alertDiv.className = 'alert';
            }, 5000);
        }

        loginForm.addEventListener('submit', async (e) => {
            e.preventDefault();
            
            const email = document.getElementById('email').value.trim();
            const password = document.getElementById('password').value;

            // Validation
            if (!email || !password) {
                showAlert('Please fill in all fields');
                return;
            }

            // Email format validation
            const emailRegex = /^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/;
            if (!emailRegex.test(email)) {
                showAlert('Please enter a valid email address');
                return;
            }

            loginBtn.disabled = true;
            loginBtn.textContent = 'Logging in...';

            const result = await authManager.loginCustomer(email, password);

            if (result.success) {
                showAlert('Login successful! Redirecting...', 'success');
                setTimeout(() => {
                    window.location.href = redirectAfterLogin;
                }, 500);
            } else {
                showAlert(result.error || 'Invalid email or password');
                loginBtn.disabled = false;
                loginBtn.textContent = 'Login';
            }
        });

        updateCartBadge();
        updateLoginHeaderState();
        setActiveNavLink();
        attachProtectedNavHandlers();
    `
    },
  ]
};

export default page;
